...
float WaterQualityCalibrator::readTDS(float temp) {
    ...
    float ec25 = ec / (1.0 + 0.02 * (temp - 25.0));  // kompensasi suhu SETELAH EC
    float tds = ec25 * TDS_FACTOR; // TDS_FACTOR biasanya 0.5
...
void WaterQualityCalibrator::calcTurbRegression() {
    float x1 = turbVoltages[0], y1 = turbValues[0];
    float x2 = turbVoltages[1], y2 = turbValues[1];
    turbSlope = (y2 - y1) / (x2 - x1);
    turbIntercept = y1 - turbSlope * x1;
    saveTurbCalibration();
}
...
int WaterQualityCalibrator::handleCommand(const String& input) {
    ...
    if (input.startsWith("TURB:") && inTurbCalib) {
        ...
        if (turbSampleCount >= 2) calcTurbRegression();
        return 2;
    }
...
void WaterQualityCalibrator::updateTDSCalibration() {
    // opsional jika ingin dipanggil di loop() untuk pembacaan otomatis TDS kalibrasi
}
